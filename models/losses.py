...
class ReconsLoss(nn.Module):
  ...
  Args:
    edge_recons: Whether to include edge reconstruction loss.
  """
    ...
    def forward(self, out_l, out_r, input_i):
      ...
      Args:
        out_l: The output of the left branch. (Not used in the current implementation)
        out_r: The output of the right branch.
        input_i: The input image.
...
class ReconsLossX(nn.Module):
  """Computes the reconstruction loss with optional edge reconstruction.
...
class DINOLoss(nn.Module):
  """Computes the DINO-ViT perceptual loss."""

    def resize_to_dino(self, feature, size = (224, 224)):
      """Resizes a feature map to the DINO input size.

      Args:
        feature: The input feature map.
        size: The target size.

      Returns:
        The resized feature map.
      """
        return F.interpolate(feature, size = size, mode='bilinear', align_corners=False)

    def calculate_crop_cls_loss(self, outputs, inputs):
      """Calculates the DINO loss for cropped images.

      Args:
        outputs: The predicted images.
        inputs: The target images.

      Returns:
        The DINO loss for cropped images.
      """
        loss = 0.0
    ...
    def forward(self, output, target):
      """Computes the DINO perceptual loss.
